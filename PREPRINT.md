...
A second failure mode emerged from independent public Cell Tracking Challenge datasets. In *Drosophila* and *Tribolium*, adding older trajectory information could improve held-out R² even when the augmented predictor itself did not transfer to the held-out acquisition. In *Tribolium* an apparent nonlinear history increment reached roughly `+0.42` at a coarse present representation while absolute augmented prediction still failed the preregistered cross-acquisition gate. A richer R7 present representation with about 46–49 relational and kinematic features likewise failed adequacy in every frozen organism/outcome task. R8 then added release-native tracking-mask geometry proxies and raw-image intensity from frames 23–25, expanding the full present representation to 139 Drosophila features and 99 Tribolium features. Gate 1 still failed in all four organism-by-outcome tasks. In Tribolium radial prediction, Random Forest and Extra Trees reached about +0.30 held-out R2 in sequence 02 but remained negative in sequence 01, so the prespecified reciprocal-acquisition gate correctly blocked promotion of the favorable fold. R9 then kept the same cohorts, frame25->40 task, models and gate while applying outcome-blind acquisition registration. The preregistered transductive domain-percentile representation made Tribolium radial prediction positive and better than the train-only mean in both reciprocal folds for Random Forest (R2 about +0.081/+0.114) and Extra Trees (+0.115/+0.224); two secondary transductive registrations also passed, whereas the target-free/invariant track, Drosophila tasks and Tribolium future speed did not. A target-row permutation control was negative. R10 therefore retested older H only in that one qualified representation. Observed mean Delta R2 was -0.0030 for Random Forest and -0.0054 for Extra Trees, with 0/100 matched H-permutation replicates passing the history gate. However, audit later found a seed-specification mismatch in that +0.30 target-SD calibration: the preregistration specified direction seed `20260830+r`, while the original implementation used `20260830+500000+r`. The original implemented-seed family yielded 10/30 joint successes. R10B reran the same calibration with the documented seed family and no other scientific change, yielding 16/30 (53.3%), still below the frozen 24/30 (80%) requirement. The history question therefore remains calibration-limited/unresolved rather than evidence of screening-off, while the exact finite sensitivity estimate is seed-family dependent. R11 separately decomposed the original implemented-seed calibration into adequacy and detection axes and evaluated its first 20 paired directions at planning-only scales 0.15, 0.30, 0.45 and 0.60 target SD. Aggregate Gate-2 detection rose from 0% to 75% while S-only adequacy preservation fell from 85% to 45%; strict joint success was 0%, 30%, 30% and 30% on that tested grid. These aggregate trends are a task-specific planning diagnostic, not a claim of replicate-wise monotonicity and not a replacement for the corrected R10B preregistration-seed result. We therefore require the empirical decision order **absolute held-out adequacy against a train-only naive baseline, then history-increment stability, then matched sensitivity calibration, then scoped biological interpretation**. A positive increment—or a single favorable acquisition—inside an inadequate transfer setting is not evidence of biological memory.

An independently audited idealized calculation (T8/T8A) clarifies when a synthetic calibration can simultaneously preserve a declared present-only R2 threshold `rho` and produce a required incremental-history R2 `delta`. In the simple oracle model, if the injected direction is S-unpredictable, is revealed by `(S,H)`, and `(S,H)` contains no additional conditional-mean information about the base residual (`E[E|S,H]=0`), compatibility holds iff `r0 >= rho/(1-delta)`, where `r0` is the unperturbed oracle present-only R2. T8A showed that the no-extra-residual-information condition is essential: without it, the augmented curve contains an additional `q=Var(E[E|S,H])/B` term. This is elementary calibration-design algebra, not a theorem about the finite-sample RF/ExtraTrees gates and not a biological or mathematical-priority claim.

A subsequent post-hoc calibration-design diagnostic (R12, independently audited in R12A) tested whether the corrected R10B failure was sensitive to the construction of the synthetic history direction. Acquisition-wise centering/scaling of the pooled Ridge-residualized direction left joint success at 16/30 with exactly the R10B success identities. A secondary acquisition-specific Ridge-residualized geometry preserved S-only adequacy in 30/30 documented directions but achieved Gate-2/joint detection in only 18/30, still below 24/30. These transforms are outcome-blind but transductive with respect to target-domain S/H; Ridge residualization does not establish literal `E[Z|S]=0`. Thus calibration geometry changes the finite failure decomposition, but the current two-acquisition/model stack still lacks the frozen sensitivity standard and the biological history question remains unresolved.

A prospectively committed follow-up surface (R13) then evaluated that acquisition-specific residualization geometry on the same 30 documented directions at 0.15, 0.30, 0.45, and 0.60 target SD, with the 0.30 arm inherited rather than refit. Joint success was **1/30, 18/30, 22/30, and 20/30** respectively. The best tested point, 22/30 at 0.45 SD, remained below the frozen 24/30 planning reference, and increasing to 0.60 SD reduced joint success. R13 therefore narrows the design lesson: this geometry improves the sensitivity surface substantially, but no preregistered grid point reaches the historical 80% target. The 0.45-SD point is not a new confirmatory threshold and cannot be used to reinterpret the biological near-zero history result. Mechanical reruns reproduce all R13 pass/fail identities exactly; numeric drift is limited to floating-point last bits.

A separate preregistered cross-embryo pilot (R15, independently audited in R15A) used the 2025 SLICE-1 Tribolium light-sheet resource rather than the original Cell Tracking Challenge acquisitions. Two Cytok8 embryos (DS0004/DS0005) were development-only and used to freeze an eight-coordinate whole-embryo four-view image-geometry state. Before validation scoring, DS0007 (a different Cytok8 subline) was fixed as the primary held-out embryo and DS0035 (Lamin #4) as a marker-domain stress test. For a four-hour-future eight-coordinate geometry vector, Random Forest and Extra Trees beat the train-only mean baseline on DS0007 with vector R2 **+0.0307** and **+0.0567**; Ridge was slightly negative (**-0.0136**). On the predeclared Lamin-domain DS0035 stress test, Ridge, Random Forest and Extra Trees were all modestly positive (**+0.0371, +0.0378, +0.0341**). Adding the four-hour-older image state reduced held-out R2 for every estimator in both embryos. R15A independently reproduced all scores and deltas to machine precision and verified the repository chronology. R15B then independently redownloaded the four exact released preview TIFFs, matched committed byte sizes and SHA-256 hashes, regenerated all four 49x49 feature tables exactly, and reproduced both adequacy gates and all primary/secondary metrics to machine precision. This is a promising cross-embryo **present-state adequacy** result, not screening-off evidence: the pilot has only two development embryos, temporally overlapping windows within embryos, and no matched known-incomplete sensitivity calibration or acquisition-level null. The history question therefore remains unresolved.
