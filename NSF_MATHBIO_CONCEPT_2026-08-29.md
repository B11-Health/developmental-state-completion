...
7. **Prospective source-simulator tomography:** after the two-context hypothesis and nonlinear decoder were frozen, a 64-phenotype source cohort achieved 100% hidden-state sign recovery across 32 complementary pairs with maximum L2 `5.15e-4`, passing all preregistered thresholds. A separately frozen 64-context extension made all five theoretically sufficient Hamming>=3 masks evaluable; all passed. The rectifier algebra is CReLU-like established mathematics; the value here is a source-rendered prospective stress test and a concrete bridge from identifiability theory to experimental design.
