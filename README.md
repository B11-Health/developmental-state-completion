...
2. **A compact current atlas representation is highly predictive in late L1, but its effective dimension depends on the decoder—and finite-state scalar coding is a trap.** One unsupervised PC matches/exceeds all 25 atlas channels for flexible nonlinear decoders, while linear Ridge needs about four PCs. Yet the cohort contains only eight distinct atlas patterns, and generic random 1-D projections also separate all eight exactly. The meaningful result is task-specific predictive compression, not intrinsic one-dimensional biology. See `REPRESENTATION_DIMENSION_CHECKPOINT_FM1_2026-08-29.md` and `FINITE_STATE_PROJECTION_TRAP_2026-08-29.md`.
...
5. **A historical 224-world experiment reported baseline-collapse / intervention-separation, but its original generator and frozen checkpoint are currently missing.** It remains part of the research history, not a presently reproducible result. The later public 128-world/two-context source bundle is a separate experiment and is not used as a substitute.
6. **Historical simulator checkpoints reported disconnected mechanism islands and accommodation-versus-accessibility gaps, but the original topology source artifacts were not recovered.** The definitions remain useful, while specific historical threshold values are now explicitly provenance-incomplete.
7. **The connected-ambiguity experiment-selection objective has a reproducible structural result: it is not generally submodular and greedy can be arbitrarily bad.** A source-derived 128-world topology proxy contains a concrete diminishing-returns violation, although greedy happens to be optimal in that tested stress case. On rooted trees with known outcomes, the objective becomes ordinary coverage and greedy regains the classical guarantee. See `MATH_VERIFICATION_M1_2026-08-29.md`.
