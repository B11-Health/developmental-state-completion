...
2. **A compact current atlas representation is highly predictive in late L1, but its effective dimension depends on the decoder.** In ancestor-grouped CV, one unsupervised current-expression PC matches or exceeds all 25 atlas channels for ExtraTrees and HistGradientBoosting across repeated lineage partitions, while linear Ridge needs about four PCs to recover the full gain. This is a task/decoder-specific representation result, **not** evidence that plant state is intrinsically one-dimensional. The 25 channels are atlas annotations rather than repeated simultaneous molecular measurements. See `REPRESENTATION_DIMENSION_CHECKPOINT_FM1_2026-08-29.md`.
