...
R=Path('/root/plant_m2_reeb_global');O=R/'five_mask_prospective_extension_source'
if O.exists():
    raise SystemExit(f'Refusing to overwrite existing {O}')
O.mkdir();fr=json.load(open(R/'FIVE_MASK_PROSPECTIVE_EXTENSION_FROZEN_2026-08-26.json'))
...
for l,g in fr['laws'].items():
 for ss in fr['new_states_to_render']:
  bits=[int(c) for c in ss];par=dict(base)
...
json.dump({'freeze_sha':fr['sha256_pre_render'],'parent_freeze_sha':fr['parent_freeze_sha'],'meta':meta},open(O/'meta.json','w'),indent=2)
for q in range(4):(R/f'fmpe_jobs_{q}').write_text('\n'.join(jobs[q::4])+'\n')
print('jobs',len(jobs),'output',O)
